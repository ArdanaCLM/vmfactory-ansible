...
class _AHVLibvirtHelper(object):
...
class _ArdanaHypervisorBase(AgentCheck):

    @staticmethod
    def _ahv_vm_state(hlh, vm):
    ...
    def ahv_vm_state(self, hlh, vm):
        return ("VM", vm, self._ahv_vm_state(hlh, vm))

    @staticmethod
    def _ahv_vnet_state(hlh, vnet):
    ...
    def ahv_vnet_state(self, hlh, vnet):
        return ("VNET", vnet, self._ahv_vnet_state(hlh, vnet))

    def ahv_vm_info(self, hlh, vm_entry):
        states = [self.ahv_vm_state(hlh, vm_entry['domain'])]
        states.extend([self.ahv_vnet_state(hlh, vnet)
        ...
        result = dict(metric=("ardana-hypervisor.vcp_vm.%s" %
                              vm_entry['domain']),
...
class ArdanaHypervisorSummary(_ARDANAHypervisorBase):
    ...
    def _build_summary_metrics(self, hlh, inst, dims):
        vm_infos = [self.ahv_vm_info(hlh, vm)
        ...
        result = dict(metric="ardana-hypervisor.vcp_vms",
                      value=int(bool(len(not_healthy))),  # 0 or 1
                      value_meta=dict(message=message, detail=detail),
                      dimensions=dims)
    ...
    def _build_vm_metrics(self, hlh, inst, dims):
        result = self.ahv_vm_info(hlh, inst)
